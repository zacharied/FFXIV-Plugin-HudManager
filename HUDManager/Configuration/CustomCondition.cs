...
public struct CustomConditionUnion
{
    ...
    public bool IsActive(Plugin plugin)
    {
        if (CurrentType == typeof(CustomCondition)) {
            ...
            if (plugin.PlayerState.ClassJob.ValueNullable is { } playerClassJob ) {
                return ClassJob!.Value.IsActivated(playerClassJob);
            }
            return false;
        }
        throw new CustomConditionUnionUndefinedException();
    ...
    /// </summary>
    public class CustomConditionUnionUndefinedException() : InvalidOperationException("no members of union are defined");
}
